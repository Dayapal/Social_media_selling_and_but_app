...
import { dummyChats, dummyOrders, platformIcons } from '../assets/assets';
...
const MyOrders = () => {
  ...
  if (!orders.length) {
    return (
      <div className='px-4 md:px-16 lg:px-24 xl:px-32'>
        <div className='max-w-2xl mx-auto mt-14 bg-white rounded-xl 
        border border-gray-200 p-8 text-center'>
          <h3 className='text-lg font-semibold '>No order yet</h3>
          <p className='text-sm text-gray-500 mt-2'>You have't purchased any listings yet.</p>
  ...
  return (
    <div className='px-4 md:px-16 lg:px-24 xl:px-32 py-6 '>
      <h2 className='text-2xl font-semibold mb-6'>My Oders</h2>
      ...
      <div className='space-y-4'>
        {
          orders.map((order) => {
            ...
            return (
              <div key={id} className='bg-white rounded-lg border border-gray-200 p-5
              flex flex-col max-w-4xl'>
                  ...
                  <div className='flex-1'>
                    <div className='flex items-start justify-between gap-4'>
                      <div>
                        ...
                        <p className='text-sm text-gray-500 mt-1'>@{listing.username} •
                          <span className='capitalize'>{listing.platform}</span>
                        </p>
                        <div className='flex gap-2 mt-2'>

                          {listing.verified && (
                            <span className='flex items-center text-xs bg-indigo-50 text-indigo-600 px-2 py-2
                            rounded-md'>
                          ...
                          {listing.monetized && (
                            <span className='flex items-center text-xs bg-green-50 text-green-600 px-2 py-2
                            rounded-md '>
                ...
                <div className='flex flex-col gap-2 items-end'>
                  <button onClick={() => setExpandedId((p) => p === id ? null : id)} className='flex items-center gap-2 bg-white border
                  border-gray-200 px-3 py-2 rounded hover:shadow text-sm ' aria-expanded={isExpanded}>
                    {isExpanded ? (
                      <>

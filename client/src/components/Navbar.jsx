...
import { BoxIcon, GrabIcon, ListIcon, MenuIcon, MessageCircleMore, MessageCircleMoreIcon, XIcon } from 'lucide-react'
        ...
        <nav className='h-20'>
                ...
                {!user ? (<div>
                    ...
                    <UserButton>
                        <UserButton.MenuItems>
                            <UserButton.Action  label='Marketplace' labelIcon={<GrabIcon 
                            size={16}/>} onClick={() =>navigate('/marketplace')}/>
                        </UserButton.MenuItems>

                        <UserButton.MenuItems>
                            <UserButton.Action  label='Messages' labelIcon={<MessageCircleMoreIcon
                            size={16}/>} onClick={() =>navigate('/messages')}/>
                        </UserButton.MenuItems>

                        <UserButton.MenuItems>
                            <UserButton.Action  label='My Listing' labelIcon={<ListIcon
            ...
            <div className={`sm:hidden fixed inset-0 ${menuOpen ? 'w-full' : 'w-0'} overflow-hidden bg-white backdrop-blur shadow-xl rounded-lg z-200 text-sm transition-all`}>
                <div className='flex flex-col items-center justify-center h-full text-xl font-semibold gap-6 p-4'>
                    ...
                    <XIcon onClick={() => setMenuOpen(false)} className='absolute size-8 right-8 top-6 text-gray-500 hover:text-gray-700' />
                </div>
            </div>
        </nav>

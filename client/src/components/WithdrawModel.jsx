...
const WithdrawModel = ({ onClose }) => {
    ...
    return (
        ...
        bg-opacity-50 z-100 flex items-center justify-center sm:p-4'>
            <div className='bg-white sm:rounded-lg shadow-2xl w-full max-w-lg
            h-screen sm:h-auto flex flex-col'>
                {/* Header  */}
                <div className='bg-linear-to-r from-indigo-600 to-indigo-600
                    ...
                    <button onClick={onClose} className='ml-4 p-1 hover:bg-white/20 hover:bg-opacity-20 rounded-lg 
                    transition-colors'>
                        <X className='w-5 h-5 ' />
                    </button>
                </div>
            ...
            gap-4 p-4 overflow-y-scroll'>
                    <div className='grid grid-cols-[2fr_3fr_1fr] items-center gap-2'>
                        Amount <input
                            onChange={(e) => setAmount(e.target.value)} value={amount} type='number' className='w-full px-2 py-1.5 text-sm
                    border border-gray-300 rounded outline-indigo-400' required />
                    ...
                    {account.map((field, index) => (
                        ...
                        items-center gap-2'>
                            ...
                            <input type={field.type} value={field.value}
                                onChange={(e) => setAccount((prev) => prev.map((c, i) => (i === index ? {
                                    ...c, value: e.target.value
                                } : c)))} className='w-full px-2 py-1.5 text-sm
                                border border-gray-300 rounded outline-indigo-400' />

                        </div>
                    ))}
                    {/* Submit Button */}
                    <button type='submit' className='bg-indigo-600 hover:bg-indigo-700
              text-white px-6 py-2 mt-4 rounded-md'>
